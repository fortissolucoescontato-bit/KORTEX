...
func TestListBackupsNewestFirst(t *testing.T) {
	home := t.TempDir()
	backupRoot := filepath.Join(home, ".kortex", "backups")
...
// TestRunArgsRestoreListIsDispatched verifies that `kortex restore --list`
...
// TestVersionBeforeSystemGuards verifies that `kortex version` returns the
...
func TestVersionBeforeSystemGuards(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "kortex") {
		t.Error("version output should contain 'kortex'")
	}
}
...
// an error whose message suggests running 'kortex help'.
func TestUnknownCommandSuggestsHelp(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "kortex help") {
		t.Error("unknown command error should suggest 'kortex help'")
	}
}

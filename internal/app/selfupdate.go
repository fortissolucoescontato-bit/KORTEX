...
const (
	envNoSelfUpdate   = "KORTEX_NO_SELF_UPDATE"
	envSelfUpdateDone = "KORTEX_SELF_UPDATE_DONE"
)
...
// selfUpdate checks for and applies a kortex update before normal dispatch.
// Returns nil on success or skip; errors are non-fatal (caller logs and continues).
//
// Guard evaluation order (per spec):
//  1. KORTEX_SELF_UPDATE_DONE=1 → skip (loop guard)
//  2. KORTEX_NO_SELF_UPDATE=1 → skip (opt-out)
...
func selfUpdate(ctx context.Context, version string, profile system.PlatformProfile, stdout io.Writer) error {
	...
	// Check for updates (only kortex).
	results := updateCheckFiltered(ctx, version, profile, []string{"kortex"})

	// Find the kortex result.
	var target *update.UpdateResult
	for i := range results {
		if results[i].Tool.Name == "kortex" {
			target = &results[i]
			break
		}
	}
	...
	for _, r := range report.Results {
		if r.ToolName == "kortex" && r.Status == upgrade.UpgradeSucceeded {
			succeeded = true
			break
		}
	}
	...
	//
	// Use exec.LookPath("kortex") rather than os.Executable() because
	// on Homebrew, os.Executable() resolves to the versioned Cellar path
	// (e.g. /opt/homebrew/Cellar/kortex/1.8.5/bin/kortex) which
	// still points to the OLD binary after upgrade. The PATH symlink
	// (/opt/homebrew/bin/kortex) is updated by Homebrew to the new
	// version, so LookPath gives us the correct binary.
	executable, err := lookPathFn("kortex")
	if err != nil {

...
func TestStartUninstall_FullRemoveHomebrewManagedBinaryAddsManualAction(t *testing.T) {
	...
	restoreExec := setOSExecutableForTest("/opt/homebrew/bin/kortex", nil)
	defer restoreExec()
	...
	if !strings.Contains(msg.Result.ManualActions[0], "brew uninstall kortex") {
...
func TestStartUninstall_FullRemoveNonBrewRemovesBinary(t *testing.T) {
	...
	restoreExec := setOSExecutableForTest("/tmp/kortex", nil)
	defer restoreExec()
	...
	if removedPath != "/tmp/kortex" {
		t.Fatalf("os.Remove path = %q, want %q", removedPath, "/tmp/kortex")
	}
}
...
func TestKiroPickerEscNonCustomWithClaudeGoesToClaudePicker(t *testing.T) {
	...
	m.Selection.Components = componentsForPreset(model.PresetFullKortex)
	m.KiroModelPicker = screens.NewKiroModelPickerState()
...
func TestStrictTDDScreenAppearsForClaudeCodeAgent(t *testing.T) {
	...
	// Cursor on a preset option (PresetFullKortex = index 0 typically).
	// Set cursor on first preset option.
	m2.Cursor = 0 // FullKortex

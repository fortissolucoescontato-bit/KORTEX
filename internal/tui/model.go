...
import (
	...
	"github.com/fortissolucoescontato-bit/kortex/internal/opencode"
...
func NewModel(detection system.DetectionResult, version string) Model {
	selection := model.Selection{
		...
		Components: componentsForPreset(model.PresetFullKortex),
	}
...
func (m Model) View() string {
	...
	case ScreenWelcome:
		...
		return screens.RenderWelcome(m.Cursor, banner, m.UpdateResults, m.UpdateCheckDone, m.hasDetectedOpenCode(), len(m.ProfileList), m.hasAgentBuilderEngines())
	case ScreenUpgrade:
	...
	case ScreenComplete:
		return screens.RenderComplete(screens.CompletePayload{
			...
			KortexCLIInstalled:        hasSelectedComponent(m.Selection.Components, model.ComponentKortexCLI),
		...
		if mode == model.UninstallModeFullRemove {
			...
			if isHomebrewManagedBinary(execPath) {
				result.ManualActions = append(result.ManualActions,
					"Homebrew-managed install detected. Run 'brew uninstall kortex' to remove the executable cleanly.")
...
func (m Model) startInstallation() (tea.Model, tea.Cmd) {
	...
	return m, tea.Batch(tickCmd(), func() (msg tea.Msg) {
		...
		registryPath := filepath.Join(homeDir(), ".config", "kortex", "custom-agents.json")

...
func RenderInstalling(progress InstallProgress, spinner string) string {
	...
	b.WriteString(styles.TitleStyle.Render("Instalando"))
	b.WriteString("\n\n")
	...
	if progress.CurrentStep != "" {
		b.WriteString(styles.SubtextStyle.Render("Atual: " + progress.CurrentStep))
		b.WriteString("\n")
	}
	b.WriteString("\n")
	...
	if progress.Done {
		...
		if progress.Failed {
			b.WriteString(styles.WarningStyle.Render(fmt.Sprintf("Concluído com erros: %d com sucesso, %d falharam", succeeded, failed)))
			b.WriteString("\n")
		} else {
			b.WriteString(styles.SuccessStyle.Render(fmt.Sprintf("Todas as %d etapas foram concluídas com sucesso.", succeeded)))
			b.WriteString("\n")
		}
		b.WriteString(styles.HelpStyle.Render("Pressione Enter para continuar."))
		b.WriteString("\n")
	}

...
func TestRenderUpgrade_AllUpToDate(t *testing.T) {
	results := []update.UpdateResult{
		{
			Tool:             update.ToolInfo{Name: "kortex"},
...
func TestRenderUpgrade_UpdatesAvailable(t *testing.T) {
	...
	if !strings.Contains(out, "kortex") {
		t.Errorf("RenderUpgrade(updates available) should contain tool name 'kortex'; got:\n%s", out)
	}

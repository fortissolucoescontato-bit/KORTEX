...
import (
	...
	codexagent "github.com/fortissolucoescontato-bit/kortex/internal/agents/codex"
	"github.com/fortissolucoescontato-bit/kortex/internal/agents/cursor"
	"github.com/fortissolucoescontato-bit/kortex/internal/agents/gemini"
	"github.com/fortissolucoescontato-bit/kortex/internal/agents/kiro"
...
func TestGoldenConfigs(t *testing.T) {
	...
	presets := []presetMapping{
		{Preset: "full-carbon", Skills: toStringSlice(skills.SkillsForPreset("full-carbon"))},
...
func TestGoldenSDD_Cursor(t *testing.T) {
	...
	// Cursor writes SDD orchestrator to ~/.cursor/rules/kortex.mdc.
	rulesFile := readTestFile(t, filepath.Join(home, ".cursor", "rules", "kortex.mdc"))
...
func TestGoldenSDD_Kiro(t *testing.T) {
	...
	// Kiro writes SDD orchestrator to ~/.kiro/steering/kortex.md
...
func TestGoldenPersona_Claude_Kortex(t *testing.T) {
	home := t.TempDir()

	result, err := persona.Inject(home, claudeAdapter(), model.PersonaKortex)
	if err != nil {
		t.Fatalf("persona.Inject(claude, carbon) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(claude, carbon) changed = false")
	}

	claudeMD := readTestFile(t, filepath.Join(home, ".claude", "CLAUDE.md"))
	assertGolden(t, "persona-claude-carbon.golden", claudeMD)

	outputStyle := readTestFile(t, filepath.Join(home, ".claude", "output-styles", "carbon.md"))
	assertGolden(t, "persona-claude-carbon-outputstyle.golden", outputStyle)

	settingsJSON := readTestFile(t, filepath.Join(home, ".claude", "settings.json"))
	assertGolden(t, "persona-claude-carbon-settings.golden", settingsJSON)
}
...
func TestGoldenPersona_OpenCode_Kortex(t *testing.T) {
	home := t.TempDir()

	result, err := persona.Inject(home, opencodeAdapter(), model.PersonaKortex)
	if err != nil {
		t.Fatalf("persona.Inject(opencode, carbon) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(opencode, carbon) changed = false")
	}

	agentsMD := readTestFile(t, filepath.Join(home, ".config", "opencode", "AGENTS.md"))
	assertGolden(t, "persona-opencode-carbon.golden", agentsMD)
}
...
func TestGoldenPersona_Windsurf_Kortex(t *testing.T) {
	home := t.TempDir()

	result, err := persona.Inject(home, windsurfAdapter(), model.PersonaKortex)
	if err != nil {
		t.Fatalf("persona.Inject(windsurf, carbon) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(windsurf, carbon) changed = false")
	}

	globalRules := readTestFile(t, filepath.Join(home, ".codeium", "windsurf", "memories", "global_rules.md"))
	assertGolden(t, "persona-windsurf-carbon.golden", globalRules)
}

func TestGoldenPersona_Kiro_Kortex(t *testing.T) {
	home := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", filepath.Join(home, ".config"))
	t.Setenv("APPDATA", filepath.Join(home, "AppData", "Roaming"))

	adapter := kiroAdapter()
	result, err := persona.Inject(home, adapter, model.PersonaKortex)
	if err != nil {
		t.Fatalf("persona.Inject(kiro, carbon) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(kiro, carbon) changed = false")
	}

	promptPath := adapter.SystemPromptFile(home)
	instructionsFile := readTestFile(t, promptPath)
	assertGolden(t, "persona-kiro-carbon.golden", instructionsFile)
}
...
func TestGoldenCombined_Windsurf(t *testing.T) {
	...
	if _, err := persona.Inject(home, windsurfAdapter(), model.PersonaKortex); err != nil {
...
func TestGoldenPersona_Antigravity_Kortex(t *testing.T) {
	home := t.TempDir()

	result, err := persona.Inject(home, antigravityAdapter(), model.PersonaKortex)
	if err != nil {
		t.Fatalf("persona.Inject(antigravity, carbon) error = %v", err)
	}
	if !result.Changed {
		t.Fatalf("persona.Inject(antigravity, carbon) changed = false")
	}

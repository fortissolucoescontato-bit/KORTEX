...
func assertKortexLanguageGuardrails(t *testing.T, text string, required []string, banned []string) {
	t.Helper()
...
func TestInjectClaudeKortexWritesSectionWithRealContent(t *testing.T) {
	home := t.TempDir()

	result, err := Inject(home, claudeAdapter(), model.PersonaKortex)
	if err != nil {
...
func TestInjectKimiKortexIncludesProjectInstructionsAndLoadedSkills(t *testing.T) {
	home := t.TempDir()

	result, err := Inject(home, kimiAdapter(), model.PersonaKortex)
	if err != nil {
	...
	// output-style.md module should contain the Kortex style content.
	...
	if !strings.Contains(string(styleContent), "Kortex Output Style") {
		t.Fatal("output-style.md missing Kortex Output Style content")
	}
...
func TestInjectClaudeKortexWritesOutputStyleFile(t *testing.T) {
	home := t.TempDir()

	_, err := Inject(home, claudeAdapter(), model.PersonaKortex)
	if err != nil {
		t.Fatalf("Inject() error = %v", err)
	}

	// Verify output-style file was written.
	stylePath := filepath.Join(home, ".claude", "output-styles", "carbon.md")
	...
	text := string(content)
	if !strings.Contains(text, "name: Kortex") {
		t.Fatal("Output style file missing YAML frontmatter 'name: Kortex'")
	}
	if !strings.Contains(text, "keep-coding-instructions: true") {
		t.Fatal("Output style file missing 'keep-coding-instructions: true'")
	}
	if !strings.Contains(text, "Kortex Output Style") {
		t.Fatal("Output style file missing 'Kortex Output Style' heading")
	}
}

func TestInjectClaudeKortexMergesOutputStyleIntoSettings(t *testing.T) {
	home := t.TempDir()
	...
	if outputStyle != "Kortex" {
		t.Fatalf("settings.json outputStyle = %q, want %q", outputStyle, "Kortex")
	}
...
func TestInjectClaudeKortexReturnsAllFiles(t *testing.T) {
	home := t.TempDir()
	...
	wantSuffixes := []string{"CLAUDE.md", "carbon.md", "settings.json"}
...
func TestInjectClaudeNeutralWritesFullPersonaWithoutRegionalLanguage(t *testing.T) {
	...
	// Should NOT have carbon-specific regional language.
...
func TestInjectOpenCodeKortexWritesAgentsFile(t *testing.T) {
	home := t.TempDir()

	result, err := Inject(home, opencodeAdapter(), model.PersonaKortex)
	if err != nil {
		t.Fatalf("Inject() error = %v", err)
	}
	if !result.Changed {
...
func TestInjectOpenCodePreservesUserPrefaceAboveATLBlock(t *testing.T) {
	...
	if strings.Contains(text, "BEGIN:kortex") {
...
func TestInjectOpenCodeReplacesExactLegacyAssetWithoutDuplication(t *testing.T) {
	...
	legacyContent := assets.MustRead("opencode/persona-carbon.md")
...
func TestInjectVSCodeNeutralPreservesManagedSections(t *testing.T) {
	...
	_, err = Inject(home, vscodeAdapter, model.PersonaKortex)
	if err != nil {
		t.Fatalf("Inject(carbon) error = %v", err)
	}
...
func TestInjectNeutralPreservesWhenMarkerAtByteZero(t *testing.T) {
	...
	markerOnly := "<!-- kortex:sdd-orchestrator -->\nSDD content\n<!-- /kortex:sdd-orchestrator -->\n"
...
func TestInjectNeutralIdempotentWithManagedSections(t *testing.T) {
	...
	if strings.Count(text, "<!-- kortex:sdd-orchestrator -->") != 1 {
...
func TestInjectClaudeIsIdempotent(t *testing.T) {
	...
	first, err := Inject(home, claudeAdapter(), model.PersonaKortex)
	if err != nil {
	...
	second, err := Inject(home, claudeAdapter(), model.PersonaKortex)
	if err != nil {
...
func TestInjectOpenCodeIsIdempotent(t *testing.T) {
	...
	first, err := Inject(home, opencodeAdapter(), model.PersonaKortex)
	if err != nil {
		t.Fatalf("Inject() first error = %v", err)
	}
	if !first.Changed {
	...
	second, err := Inject(home, opencodeAdapter(), model.PersonaKortex)
	if err != nil {
		t.Fatalf("Inject() second error = %v", err)
	}
	if second.Changed {
...
func TestInjectWindsurfIsIdempotent(t *testing.T) {
	...
	first, err := Inject(home, windsurfAdapter, model.PersonaKortex)
	if err != nil {
		t.Fatalf("Inject() first error = %v", err)
	}
	if !first.Changed {
	...
	second, err := Inject(home, windsurfAdapter, model.PersonaKortex)
	if err != nil {
		t.Fatalf("Inject() second error = %v", err)
	}
	if second.Changed {
...
func TestInjectCursorKortexWritesRulesFileWithRealContent(t *testing.T) {
	home := t.TempDir()
	...
	result, injectErr := Inject(home, cursorAdapter, model.PersonaKortex)
	if injectErr != nil {
		t.Fatalf("Inject(cursor) error = %v", injectErr)
	}

	if !result.Changed {
		t.Fatalf("Inject(cursor, carbon) changed = false")
	}

	// Verify the generic persona content was used — not just neutral one-liner.
	path := filepath.Join(home, ".cursor", "rules", "kortex.mdc")
...
func TestInjectGeminiKortexWritesSystemPromptWithRealContent(t *testing.T) {
	home := t.TempDir()
	...
	result, injectErr := Inject(home, geminiAdapter, model.PersonaKortex)
	if injectErr != nil {
		t.Fatalf("Inject(gemini) error = %v", injectErr)
	}

	if !result.Changed {
		t.Fatal("Inject(gemini, carbon) changed = false")
	}
...
func TestInjectVSCodeKortexWritesInstructionsFile(t *testing.T) {
	home := t.TempDir()
	...
	result, injectErr := Inject(home, vscodeAdapter, model.PersonaKortex)
	if injectErr != nil {
		t.Fatalf("Inject(vscode) error = %v", injectErr)
	}

	if !result.Changed {
		t.Fatal("Inject(vscode, carbon) changed = false")
	}
...
// legacyClaudePersonaBlock simulates a Kortex persona block that was written
...
func TestInjectClaudeAutoHealsStaleFreeTextPersona(t *testing.T) {
	...
	if !strings.Contains(text, "<!-- kortex:sdd -->") {
	...
	if firstPersonaIdx != lastPersonaIdx {
		...
		openMarkerIdx := strings.Index(text, "<!-- kortex:persona -->")
...
func TestInjectVSCodeCleansLegacyGitHubPersonaFile(t *testing.T) {
	...
	// Plant an old-style Kortex persona file at the legacy path.
...
func TestInjectVSCodePreservesNonPersonaGitHubFile(t *testing.T) {
	...
	// Kortex persona) — it must NOT be deleted.
	legacyPath := filepath.Join(home, ".github", "copilot-instructions.md")
	if err := os.MkdirAll(filepath.Dir(legacyPath), 0o755); err != nil {
		t.Fatalf("MkdirAll error = %v", err)
	}
	...
	_, injectErr := Inject(home, vscodeAdapter, model.PersonaKortex)
	if injectErr != nil {
		t.Fatalf("Inject(vscode) error = %v", injectErr)
	}
...
func TestNeutralAndKortexToneSectionsMatch(t *testing.T) {
	neutral := assets.MustRead("generic/persona-neutral.md")
	carbon := assets.MustRead("generic/persona-carbon.md")
	...
	carbonTone := extractSection(carbon, "Tone")

	if neutralTone != carbonTone {
		t.Fatalf("## Tone sections diverged:\nneutral:\n%s\ncarbon:\n%s", neutralTone, carbonTone)
	}
}
...
func TestInjectVSCodeIdempotentAfterHeal(t *testing.T) {
	...
	first, err := Inject(home, vscodeAdapter, model.PersonaKortex)
	if err != nil {
		t.Fatalf("Inject() first error = %v", err)
	}
	if !first.Changed {
	...
	second, err := Inject(home, vscodeAdapter, model.PersonaKortex)
	if err != nil {
		t.Fatalf("Inject() second error = %v", err)
	}
	if second.Changed {

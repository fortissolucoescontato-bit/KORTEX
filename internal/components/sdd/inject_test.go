...
import (
	...
	windsurfagent "github.com/fortissolucoescontato-bit/kortex/internal/agents/windsurf"
...
func TestInjectClaudeCustomModelAssignments(t *testing.T) {
	...
	if !strings.Contains(text, "<!-- kortex:sdd-model-assignments -->") {
...
func TestInjectCursorWritesSDDOrchestratorAndSkills(t *testing.T) {
	...
	promptPath := filepath.Join(home, ".cursor", "rules", "kortex.mdc")
...
func TestInjectKimiWritesNativeAgentFilesAndGlobalSkills(t *testing.T) {
	...
	rootAgentPath := filepath.Join(home, ".kimi", "agents", "carbon.yaml")
	...
	if !strings.Contains(rootText, "name: carbon") {
		t.Fatal("carbon.yaml should define a named root custom agent")
	}
	if strings.Contains(rootText, "kimi_cli.tools.agent:Agent") {
		t.Fatal("carbon.yaml should inherit Kimi's default tool set instead of hardcoding the old Agent tool path")
	}
	if !strings.Contains(rootText, "../KIMI.md") {
		t.Fatal("carbon.yaml should load the installed KIMI.md system prompt")
	}
...
<!-- END:kortex -->`
	...
	if strings.Contains(text, "<!-- END:kortex -->") {
...
func TestInjectSharedDirCreatedWithAllFiles(t *testing.T) {
	...
	for _, want := range []string{"persistence-contract.md", "engram-convention.md", "kortex-convention.md", "openspec-convention.md", "sdd-phase-common.md", "skill-resolver.md"} {
		if !names[want] {
...
func TestStripBareOrchestratorSection_DoesNotStripIfMarkersPresent(t *testing.T) {
	...
	// the `!strings.Contains(existing, "<!-- kortex:sdd-orchestrator -->")` guard
...
// the <!-- kortex:strict-tdd-mode --> marker with its content.
func TestInjectStrictTDDEnabledInjectsMarkerIntoClaude(t *testing.T) {
	...
	text := string(content)
	if !strings.Contains(text, "<!-- kortex:strict-tdd-mode -->") {
		t.Fatal("CLAUDE.md missing <!-- kortex:strict-tdd-mode --> open marker")
	}
	if !strings.Contains(text, "<!-- /kortex:strict-tdd-mode -->") {
		t.Fatal("CLAUDE.md missing <!-- /kortex:strict-tdd-mode --> close marker")
	}
...
func TestInjectClaudeDoesNotStripMarkedSection(t *testing.T) {
	...
	if !strings.Contains(string(after1), "<!-- kortex:sdd-orchestrator -->") {
...
func TestInjectWindsurf_WorkflowsFoundFromSubdirectory(t *testing.T) {
	...
	// Simulate running kortex from a subdirectory inside that project.
...
// ---------------------------------------------------------------------------
// carbon agent model mirroring from sdd-orchestrator
// ---------------------------------------------------------------------------

// TestInjectOpenCodeMultiModeMirrorsOrchestratorModelToKortex verifies that
// when sdd-orchestrator has an explicit TUI model assignment and the carbon
// agent already exists in opencode.json (persona installed), the orchestrator
// model is mirrored to the carbon agent.
func TestInjectOpenCodeMultiModeMirrorsOrchestratorModelToKortex(t *testing.T) {
	home := t.TempDir()
	mockNoPackageManager(t)
	...
	// Pre-existing opencode.json with carbon agent (persona installed).
	existing := `{
  "agent": {
    "carbon": {
      "mode": "primary"
	...
	// carbon must have the same model as sdd-orchestrator (mirrored).
	carbonAgent, ok := agentMap["carbon"].(map[string]any)
	if !ok {
		t.Fatal("carbon agent not found or wrong type")
	}
	if m, _ := carbonAgent["model"].(string); m != "openai/gpt-4o" {
		t.Fatalf("carbon model = %q, want %q (should mirror sdd-orchestrator)", m, "openai/gpt-4o")
	}
}

// TestInjectOpenCodeMultiModeDoesNotInjectKortexIfNotInstalled verifies that
// when the carbon agent does NOT exist in opencode.json (persona not installed),
// the orchestrator model is NOT mirrored to a carbon entry.
func TestInjectOpenCodeMultiModeDoesNotInjectKortexIfNotInstalled(t *testing.T) {
	home := t.TempDir()
	mockNoPackageManager(t)
	...
	// carbon must NOT appear — persona is not installed.
	if carbonRaw, exists := agentMap["carbon"]; exists {
		// If it somehow exists, it must not have a model field.
		if carbonMap, ok := carbonRaw.(map[string]any); ok {
			if _, hasModel := carbonMap["model"]; hasModel {
				t.Fatal("carbon should NOT have a model field when persona is not installed")
			}
		}
	}
}

...
type InjectOptions struct {
	...
	// <!-- kortex:strict-tdd-mode --> marker section is injected into
...
//
// Walking upward means users can run kortex from any subdirectory of their
...
func Inject(homeDir string, adapter agents.Adapter, sddMode model.SDDModeID, options ...InjectOptions) (InjectionResult, error) {
	...
	// findProjectRoot walks upward from WorkspaceDir so kortex can be
...
func hasLegacyBareOrchestrator(content string) bool {
	markedIdx := strings.Index(content, "<!-- kortex:sdd-orchestrator -->")
	if markedIdx >= 0 {
...
//   - end at the next managed marker ("<!-- kortex:") if present, else EOF
...
func stripBareOrchestratorForFilePrompt(content string) string {
	if markedIdx := strings.Index(content, "<!-- kortex:sdd-orchestrator -->"); markedIdx >= 0 {
		prefix := content[:markedIdx]
	...
	if rel := strings.Index(content[start:], "<!-- kortex:"); rel >= 0 {
		end = start + rel
	}
...
func injectMarkdownSections(homeDir string, adapter agents.Adapter, assignments map[string]model.ClaudeModelAlias) (InjectionResult, error) {
	...
	if hasSDDOrchestrator(existing) && !strings.Contains(existing, "<!-- kortex:sdd-orchestrator -->") {
...
func injectClaudeModelAssignments(content string, assignments map[string]model.ClaudeModelAlias) (string, error) {
	const openMarker = "<!-- kortex:sdd-model-assignments -->"
	const closeMarker = "<!-- /kortex:sdd-model-assignments -->"
...
func injectModelAssignments(overlayBytes []byte, assignments map[string]model.ModelAssignment, rootModelID string, existingAgentKeys map[string]bool) ([]byte, error) {
	...
	// Mirror sdd-orchestrator model to carbon — both are primary conductors in OpenCode.
	// carbon is defined by the persona overlay (not the SDD overlay), so we inject
	// its model field here to prevent silent runtime inheritance.
	// Guard: only inject if carbon already exists in opencode.json (persona was installed)
	...
	if orchAssignment, hasOrch := assignments["sdd-orchestrator"]; hasOrch &&
		...
		existingAgentKeys["carbon"] {
		if _, exists := agents["carbon"]; !exists {
			agents["carbon"] = map[string]any{}
		}
		if carbonMap, ok := agents["carbon"].(map[string]any); ok {
			carbonMap["model"] = orchAssignment.FullID()
		}
	}

...
func TestReadCurrentModelAssignments(t *testing.T) {
  ...
  "agent": {
    ...
    "carbon": { "model": "anthropic:claude-sonnet-4-20250514" }
  }
}`
	...
	// "carbon" is not an SDD phase — it should NOT be in the result
	if _, ok := got["carbon"]; ok {
		t.Error("non-SDD agent 'carbon' should not be in result")
	}
}

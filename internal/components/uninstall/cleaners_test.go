...
func TestRemoveManagedPersonaPreamble_PreservesManagedSuffix(t *testing.T) {
	...
	if strings.Contains(updated, "name: Kortex Persona") || strings.Contains(updated, "## Personality") {
		t.Fatalf("managed persona preamble still present:\n%s", updated)
	}
	if !strings.HasPrefix(updated, "<!-- kortex:sdd-orchestrator -->") {

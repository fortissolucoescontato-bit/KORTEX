...
func (s *Service) CompleteUninstall() (Result, error) {
	...
	result.ManualActions = append(result.ManualActions, "To completely remove kortex from your system, delete the executable (e.g., rm -f $(which kortex))")
	return result, nil
}
...
func (s *Service) buildPlan(agentIDs []model.AgentID, componentIDs []model.ComponentID) (plan, error) {
	...
	// No specific file path needed for backup targets in SQLite mode, 
	// but we snapshot the state DB directory if needed.
	// (SQLite itself is handled via snapshotter Create).
...
func (s *Service) componentOperations(adapter agents.Adapter, componentID model.ComponentID) ([]operation, []string, error) {
	...
	case model.ComponentPersona:
		...
		if adapter.SupportsOutputStyles() {
			path := filepath.Join(adapter.OutputStyleDir(homeDir), "carbon.md")
			targets = append(targets, path)
		...
		if path := adapter.SettingsPath(homeDir); path != "" {
			...
			if adapter.Agent() == model.AgentOpenCode {
				jsonPaths = append(jsonPaths, jsonPath{"agent", "carbon"})
			}
	...
	case model.ComponentKortexCLI:
		...
		ops = append(ops, removeDirIfEmpty(filepath.Dir(kortex.ConfigPath(homeDir))))
	default:
...
func globalBackupTargets(homeDir string) []string {
	return []string{
		kortex.ConfigPath(homeDir),
		kortex.AgentsTemplatePath(homeDir),
	}
}
...
func updateStateAfterUninstall(homeDir string, toRemove []model.AgentID) ([]model.AgentID, error) {
	...
	currentAgents, err := mgr.GetInstalledAgents()
	if err != nil {
		return nil, fmt.Errorf("get installed agents: %w", err)
	}
	...
	kept := make([]string, 0, len(currentAgents))
	removed := make([]model.AgentID, 0, len(toRemove))
	for _, installed := range currentAgents {
	...
	if err := mgr.SetInstalledAgents(kept); err != nil {
		return nil, fmt.Errorf("update installed agents: %w", err)
	}

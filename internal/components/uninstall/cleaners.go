...
func removeManagedPersonaPreamble(content string) (string, bool) {
	...
	markerIdx := strings.Index(normalized, "<!-- kortex:")
...
func looksLikeManagedPersonaPrefix(prefix string) bool {
	if strings.Contains(prefix, "name: Kortex Carbon Persona") && strings.Contains(prefix, "description: Teaching-oriented persona") {
		return true
	}
...
func cleanCodexTOML(content string) (string, bool) {
	normalized := strings.ReplaceAll(content, "\r\n", "\n")
	updated := removeTOMLTable(normalized, "mcp_servers.kortex-engram")

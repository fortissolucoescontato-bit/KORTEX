...
// legacyPersonaFingerprints are substrings that appear in the Kortex persona
...
// StripLegacyPersonaBlock removes a free-text Kortex persona block that was
// written to a markdown file outside of <!-- kortex: --> markers.
//
...
// <!-- kortex: --> marker). A fingerprint that exists only inside a marker
...
const (
	atlBeginMarker = "<!-- BEGIN:kortex -->"
	atlEndMarker   = "<!-- END:kortex -->"
)
...
// written by the standalone ATL installer before kortex superseded it.
// The block is wrapped in <!-- BEGIN:kortex --> / <!-- END:kortex -->
// HTML comment markers. Its content is now provided by the canonical
// <!-- kortex:sdd-orchestrator --> section, so keeping both wastes ~150
...
// Markers use HTML comments: <!-- kortex:SECTION_ID --> ... <!-- /kortex:SECTION_ID -->

...
func TestBuildConfig(t *testing.T) {
	...
	if !strings.HasPrefix(config, "# Kortex Guardian Angel") {
...
func TestConfigPath(t *testing.T) {
	...
	expected := filepath.Join("/home/testuser", ".config", "kortex", "config")
	if path != expected {
...
func TestAgentsTemplatePath(t *testing.T) {
	...
	expected := filepath.Join("/home/testuser", ".config", "kortex", "AGENTS.md")
	if path != expected {
...
func TestFilesWritten(t *testing.T) {
	result := InjectionResult{
		ConfigFile: "/home/user/.config/kortex/config",
		AgentsFile: "/home/user/.config/kortex/AGENTS.md",
	}
...
func TestPostInstallMessages(t *testing.T) {
	...
	if !strings.Contains(msgs[0], "kortex install") {
		t.Error("first message should mention kortex install")
	}

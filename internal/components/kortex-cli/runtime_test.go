...
func TestRuntimeBinDir(t *testing.T) {
	tests := []struct {
		...
		{"/home/user", filepath.Join(".local", "share", "kortex", "bin")},
		{"/root", filepath.Join(".local", "share", "kortex", "bin")},
	}
...
func TestRuntimePS1Path(t *testing.T) {
	tests := []struct {
		...
		{"/home/user", filepath.Join("bin", "kortex.ps1")},
		{"/root", filepath.Join("bin", "kortex.ps1")},
	}
	for _, tc := range tests {
...
func TestEnsurePowerShellShimCreatesFileWhenMissing(t *testing.T) {
	...
	if !strings.Contains(text, "Get-Command git") {
		t.Fatalf("kortex.ps1 missing expected content, got: %s", text)
	}
}
...
func TestEnsurePowerShellShimOverwritesStaleShim(t *testing.T) {
	...
	if string(content) == stale {
		t.Fatalf("EnsurePowerShellShim did not overwrite stale kortex.ps1")
	}
	if !strings.Contains(string(content), "Get-Command git") {
		t.Fatalf("overwritten kortex.ps1 missing expected embedded content")
	}
}

// TestEnsurePowerShellShimIsNoOpWhenContentMatches verifies idempotency:
// when kortex.ps1 already contains the correct embedded content,
...
// TestAssetKortexCLIPS1IsEmbeddedAndReadable verifies the kortex.ps1 asset is
// correctly embedded and can be read via the assets package.
func TestAssetKortexCLIPS1IsEmbeddedAndReadable(t *testing.T) {
	content, err := assets.Read("kortex/kortex.ps1")
	if err != nil {
		t.Fatalf("assets.Read(\"kortex/kortex.ps1\") error = %v", err)
	}
	if content == "" {
		t.Fatal("assets.Read(\"kortex/kortex.ps1\") returned empty content")
	}
	if !strings.Contains(content, "Get-Command git") {
		t.Fatalf("embedded kortex.ps1 missing expected content, got: %s", content)
	}
}

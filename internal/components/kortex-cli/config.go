...
// InjectionResult describes all files written by the KortexCLI config injection.
...
// PostInstallMessages returns user guidance after KortexCLI setup.
func PostInstallMessages() []string {
	return []string{
		"Run `kortex install` in any git project to set up the pre-commit hook",
		"Copy `~/.config/kortex/AGENTS.md` to your project root and customize it",
	}
}

// ProviderForAgents auto-detects the KortexCLI provider based on selected agents.
...
// BuildConfig generates the shell-sourced KortexCLI global config content.
func BuildConfig(provider string) []byte {
	var b strings.Builder
	b.WriteString("# Kortex Guardian Angel Configuration\n")
	b.WriteString("# Generated by kortex\n")
	b.WriteString("\n")
...
// ConfigPath returns the KortexCLI global config file path.
func ConfigPath(homeDir string) string {
	return filepath.Join(homeDir, ".config", "kortex", "config")
}

// AgentsTemplatePath returns the starter AGENTS.md template path.
func AgentsTemplatePath(homeDir string) string {
	return filepath.Join(homeDir, ".config", "kortex", "AGENTS.md")
}

// Inject writes the KortexCLI global config and starter AGENTS.md template.
...
func Inject(homeDir string, agentIDs []model.AgentID) (InjectionResult, error) {
	...
	if err != nil {
		return InjectionResult{}, fmt.Errorf("write kortex config: %w", err)
	}

	agentsContent, err := assets.Read("kortex/AGENTS.md")
	if err != nil {
		return InjectionResult{}, fmt.Errorf("read embedded kortex AGENTS.md template: %w", err)
	}

	agentsPath := AgentsTemplatePath(homeDir)
	agentsResult, err := filemerge.WriteFileAtomic(agentsPath, []byte(agentsContent), 0o644)
	if err != nil {
		return InjectionResult{}, fmt.Errorf("write kortex AGENTS.md template: %w", err)
	}

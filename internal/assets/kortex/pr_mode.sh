...
# ============================================================================
# Kortex Guardian Angel - PR Mode Functions
# ============================================================================
...
detect_base_branch() {
  ...
  echo "Set PR_BASE_BRANCH in your .kortex config to specify the base branch." >&2
  return 1
}
...
get_pr_files() {
  ...
  echo "$changed" | while IFS= read -r file; do
    ...
    if [[ "$match" == true && "$excluded" == false ]]; then
      ...
      # Kortex CLI_SKIP_FILE_CHECK allows unit tests to skip this check
      if [[ -n "${Kortex CLI_SKIP_FILE_CHECK:-}" ]] || [[ -f "$file" ]]; then
        echo "$file"
      fi
    fi
  done

...
| Multi-module, >300 lines, uncertain scope | **Large** | Full SDD: Kiro specs + Kortex-Engram persistence + phase gates |
| User says "use SDD" or "hazlo con SDD" | **Large** | Full SDD regardless of size |

**When in doubt**: Ask the user. "This looks medium-sized. Want to use Kiro's native spec workflow, or full SDD with Kortex-Engram artifacts?"
...
- **`kortex-engram`**: Fast, no files created. Artifacts live in kortex-engram only. Best for solo work and quick iteration. Note: re-running a phase overwrites the previous version.
...
Kortex writes to the global steering file (`~/.kiro/steering/kortex.md`) — treat it as your active working context alongside your project steering files.
...
Each SDD phase is delegated to its native Kiro subagent. Invoke with `/sdd-<phase>` or by instructing Kiro to use the subagent explicitly. Each subagent runs in its own context window, reads the required artifacts, executes its skill, writes its artifact to Kortex-Engram, and returns a result. The orchestrator synthesizes the result and decides the next step.
...
For phases with required dependencies, retrieve artifacts from Kortex-Engram using topic keys before starting the phase. Do NOT rely on conversation history alone — conversation context is lossy across sessions.
...
Convention files under the global skills directory (global) or `.agent/skills/_shared/` (workspace): `kortex-engram-convention.md`, `kortex-convention.md`, `persistence-contract.md`, `openspec-convention.md`.

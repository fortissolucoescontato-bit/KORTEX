...
func TestBashScriptPathWindowsUsesForwardSlashes(t *testing.T) {
	profile := system.PlatformProfile{OS: "windows", PackageManager: "winget"}
	got := bashScriptPath(profile, `C:\Users\jorge\AppData\Local\Temp\carbon-guardian-angel\install.sh`)
	want := "C:/Users/jorge/AppData/Local/Temp/carbon-guardian-angel/install.sh"
	if got != want {
...
func TestResolveComponentInstall(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "kortex on darwin uses brew tap and reinstall",
			profile:   system.PlatformProfile{OS: "darwin", PackageManager: "brew"},
			component: model.ComponentKortexCLI,
			want:      CommandSequence{{"brew", "tap", "fortissolucoescontato-bit/homebrew-tap"}, {"brew", "reinstall", "kortex"}},
		},
		{
			name:      "kortex on ubuntu uses git clone and install.sh",
			profile:   system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroUbuntu, PackageManager: "apt"},
			component: model.ComponentKortexCLI,
			want: CommandSequence{
				{"rm", "-rf", "/tmp/carbon-guardian-angel"},
				{"git", "clone", "https://github.com/fortissolucoescontato-bit/carbon-guardian-angel.git", "/tmp/carbon-guardian-angel"},
				{"bash", "/tmp/carbon-guardian-angel/install.sh"},
			},
		},
		{
			name:      "kortex on arch uses git clone and install.sh",
			profile:   system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroArch, PackageManager: "pacman"},
			component: model.ComponentKortexCLI,
			want: CommandSequence{
				{"rm", "-rf", "/tmp/carbon-guardian-angel"},
		...
		{
			name:      "kortex on fedora uses git clone and install.sh",
			profile:   system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroFedora, PackageManager: "dnf"},
			component: model.ComponentKortexCLI,
			want: CommandSequence{
				{"rm", "-rf", "/tmp/carbon-guardian-angel"},
		...
		{
			name:      "kortex on windows cleans temp dir and uses git bash",
			profile:   system.PlatformProfile{OS: "windows", PackageManager: "winget"},
			component: model.ComponentKortexCLI,
			want: CommandSequence{
				{"powershell", "-NoProfile", "-Command", fmt.Sprintf("Remove-Item -Recurse -Force -ErrorAction SilentlyContinue '%s'; exit 0", filepath.Join(os.TempDir(), "carbon-guardian-angel"))},
				{"git", "clone", "https://github.com/fortissolucoescontato-bit/carbon-guardian-angel.git", filepath.Join(os.TempDir(), "carbon-guardian-angel")},
				{gitBashPath(), bashScriptPath(system.PlatformProfile{OS: "windows"}, filepath.Join(os.TempDir(), "carbon-guardian-angel", "install.sh"))},
			},
		},
		{

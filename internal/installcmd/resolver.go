...
// resolveKortexCLIInstall returns the correct install command sequence for KortexCLI per platform.
// - darwin: brew tap + brew install (via fortissolucoescontato-bit/homebrew-tap)
// - linux: git clone + install.sh (KortexCLI is a pure Bash project, NOT a Go module)
func resolveKortexCLIInstall(profile system.PlatformProfile) (CommandSequence, error) {
	switch profile.PackageManager {
	case "brew":
		return CommandSequence{
			{"brew", "tap", "fortissolucoescontato-bit/homebrew-tap"},
			{"brew", "reinstall", "kortex"},
		}, nil
	case "apt", "pacman", "dnf":
		const tmpDir = "/tmp/carbon-guardian-angel"
		return CommandSequence{
			{"rm", "-rf", tmpDir},
			{"git", "clone", "https://github.com/fortissolucoescontato-bit/carbon-guardian-angel.git", tmpDir},
	...
	case "winget":
		...
		cloneDst := filepath.Join(os.TempDir(), "carbon-guardian-angel")
		bash := gitBashPath()
		return CommandSequence{
			{"powershell", "-NoProfile", "-Command", fmt.Sprintf("Remove-Item -Recurse -Force -ErrorAction SilentlyContinue '%s'; exit 0", cloneDst)},
			{"git", "clone", "https://github.com/fortissolucoescontato-bit/carbon-guardian-angel.git", cloneDst},
			{bash, bashScriptPath(profile, filepath.Join(cloneDst, "install.sh"))},
		}, nil
	default:
		return nil, fmt.Errorf(
			"unsupported platform for kortex: os=%q distro=%q pm=%q",
			profile.OS, profile.LinuxDistro, profile.PackageManager,
		)
	}
}

...
// Example: <!-- kortex:custom-agent:my-skill -->
const markerFormat = "<!-- kortex:custom-agent:%s -->"
...
func buildSDDBlock(agent *GeneratedAgent, marker string) string {
	...
	endMarker := fmt.Sprintf("<!-- /kortex:custom-agent:%s -->", agent.Name)
	return marker + "\n" + body + endMarker
}
...
func replaceBlock(content, marker, newBlock string) string {
	endMarker := fmt.Sprintf("<!-- /kortex:custom-agent:%s -->", extractName(marker))
...
// Example: "<!-- kortex:custom-agent:my-skill -->" → "my-skill"
func extractName(marker string) string {
	prefix := "<!-- kortex:custom-agent:"
	suffix := " -->"

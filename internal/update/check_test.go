...
func TestDetectInstalledVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "kortex uses build var",
			tool:         ToolInfo{Name: "kortex", DetectCmd: nil},
			currentBuild: "1.5.0",
			wantVersion:  "1.5.0",
		},
		{
			name:         "kortex dev build",
			tool:         ToolInfo{Name: "kortex", DetectCmd: nil},
			currentBuild: "dev",
			wantVersion:  "dev",
		},
		...
		{
			name: "kortex not installed",
			tool: ToolInfo{Name: "kortex", DetectCmd: []string{"kortex", "--version"}},
			lookPathFn: func(string) (string, error) {
		...
		},
		{
			...
			execCommandFn: func(name string, args ...string) *exec.Cmd {
				return exec.Command("echo", "kortex - no version info")
			},
			wantVersion: "",
		},
		{
...
func TestFetchLatestRelease_GithubToken(t *testing.T) {
	...
	if gotUserAgent != "kortex-update-check" {
		t.Fatalf("User-Agent = %q, want %q", gotUserAgent, "kortex-update-check")
	}
}
...
func TestCheckAll_NetworkError(t *testing.T) {
	...
	// kortex has no DetectCmd, so it gets currentBuildVersion "1.0.0" as local
	// but fetch fails → CheckFailed (it has a local version).
	if results[0].Status != CheckFailed {
		t.Fatalf("kortex status = %q, want %q", results[0].Status, CheckFailed)
	}
	if results[0].Err == nil {
		t.Fatalf("kortex expected error, got nil")
	}
...
func TestRegistryContents(t *testing.T) {
	...
	// kortex must have nil DetectCmd.
...
func TestCheckAll_DevVersion(t *testing.T) {
	...
	// Override only the first tool (kortex) by running CheckAll with "dev".
	origTools := Tools
	t.Cleanup(func() {
		httpClient = origClient
		lookPath = origLookPath
	...
	// Restrict to just kortex to isolate the test.
	...
	if results[0].Status != DevBuild {
		t.Fatalf("kortex dev status = %q, want %q", results[0].Status, DevBuild)
	}
}
...
// and that the dev-build sentinel causes kortex to be reported as DevBuild.
...
// TestCheckFiltered_DevBuildSemanticsForKortex verifies the design requirement:
// when the running kortex binary reports version "dev", it is identified as a
...
func TestCheckFiltered_DevBuildSemanticsForKortex(t *testing.T) {
	...
	lookPath = func(string) (string, error) { return "", fmt.Errorf("not found") }
	execCommand = func(name string, args ...string) *exec.Cmd { return exec.Command("false") }
	Tools = []ToolInfo{Tools[0]} // kortex only
	...
	if r.Tool.Name != "kortex" {
		t.Fatalf("tool = %q, want kortex", r.Tool.Name)
	}
...
func TestNoUpdatesPath(t *testing.T) {
	...
	if results[1].Status != NotInstalled {
		t.Fatalf("kortex status = %q, want NotInstalled", results[1].Status)
	}
}

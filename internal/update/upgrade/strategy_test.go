...
// self-replace for kortex is NOT attempted in Phase 1 — it must return a
...
func TestRunStrategy_BinaryWindowsSelfUpdateSkipped(t *testing.T) {
	...
	r := update.UpdateResult{
		...
		LatestVersion: "1.5.0",
		ReleaseURL:    "https://github.com/fortissolucoescontato-bit/kortex/releases/tag/v1.5.0",
	}
...
func TestEffectiveMethod(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			tool:    update.ToolInfo{Name: "kortex", InstallMethod: update.InstallBinary},
			profile: system.PlatformProfile{PackageManager: "brew"},
			want:    update.InstallBrew,
		},
		{
			name:    "brew profile overrides script",
			tool:    update.ToolInfo{Name: "kortex", InstallMethod: update.InstallScript},
			profile: system.PlatformProfile{PackageManager: "brew"},
			want:    update.InstallBrew,
		},
		{
...
func TestManualFallbackHint(t *testing.T) {
	r := update.UpdateResult{
		...
		LatestVersion: "1.5.0",
		UpdateHint:    "See https://github.com/fortissolucoescontato-bit/kortex/releases",
	}
	profile := system.PlatformProfile{OS: "windows", PackageManager: "winget"}
...
func TestBrewUpgrade_RunsUpdateBeforeUpgrade(t *testing.T) {
	...
	err := brewUpgrade(context.Background(), "kortex")
	if err != nil {
...
func TestBrewUpgrade_UpdateFailureIsNonFatal(t *testing.T) {
	...
	execCommand = func(name string, args ...string) *exec.Cmd {
		...
		return exec.Command("echo", "Upgraded kortex")
	}
...
// --- TestKortexCLIScriptUpgradeUsesGitClone ---

// TestKortexCLIScriptUpgradeUsesGitClone verifies that kortexScriptUpgrade:
// 1. First calls `git clone <repo-url> /tmp/carbon-guardian-angel`
// 2. Then calls `bash /tmp/carbon-guardian-angel/install.sh`
// — not `bash -c <script-content>` like the generic scriptUpgrade.
func TestKortexCLIScriptUpgradeUsesGitClone(t *testing.T) {
	origExecCommand := execCommand
	...
	err := kortexScriptUpgrade(context.Background(), r)
	if err != nil {
		t.Fatalf("kortexScriptUpgrade: unexpected error: %v", err)
	}
	...
	for _, a := range cloneArgs {
		if containsAny(a, "carbon-guardian-angel") {
			foundRepoURL = true
			break
		}
	}
	if !foundRepoURL {
		t.Errorf("git clone args %v should include the repo URL (carbon-guardian-angel)", cloneArgs)
	}
...
// --- TestKortexCLIScriptUpgradeWindowsManualFallback ---

// TestKortexCLIScriptUpgradeWindowsManualFallback verifies that on Windows,
// kortexScriptUpgrade returns a ManualFallbackError without calling exec.
func TestKortexCLIScriptUpgradeWindowsManualFallback(t *testing.T) {
	origExecCommand := execCommand
	t.Cleanup(func() { execCommand = origExecCommand })
	...
	err := kortexScriptUpgradeForOS(context.Background(), r, "windows")
	if err == nil {
	...
	if execCalled {
		t.Errorf("exec should NOT be called on Windows for kortexScriptUpgrade")
	}
}

// --- TestRunStrategy_KortexCLIUsesGitClone ---

// TestRunStrategy_KortexCLIUsesGitClone verifies that when runStrategy is called with
// a KortexCLI tool (InstallScript), it routes to kortexScriptUpgrade (git clone approach)
// rather than the generic scriptUpgrade (bash -c <content>).
func TestRunStrategy_KortexCLIUsesGitClone(t *testing.T) {
	origExecCommand := execCommand
	origDetectOS := detectOS
	t.Cleanup(func() {
		execCommand = origExecCommand
	...
	err := runStrategy(context.Background(), r, profile)
	if err != nil {
		t.Fatalf("runStrategy KortexCLI: unexpected error: %v", err)
	}
...
func TestInstallScriptURL(t *testing.T) {
	url := installScriptURL("fortissolucoescontato-bit", "carbon-guardian-angel")
	if url != "https://raw.githubusercontent.com/fortissolucoescontato-bit/carbon-guardian-angel/main/install.sh" {

...
func TestFindBinaryInTar(t *testing.T) {
	...
	entries := []struct {
		...
		{"kortex_1.5.0_darwin_arm64/kortex", content}, // binary in subdir
	}
	...
	outPath := filepath.Join(t.TempDir(), "kortex")
	...
	err := downloadBinary(context.Background(), server.URL+"/release.tar.gz", "kortex", outPath)
	if err != nil {
		t.Fatalf("downloadBinary: %v", err)
	}

...
func TestExecute_DevBuildIsSkipped(t *testing.T) {
	...
	// kortex (DevBuild) MUST appear as UpgradeSkipped with a ManualHint.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "kortex" {
			r := report.Results[i]
			devResult = &r
		}
	}
	if devResult == nil {
		t.Fatalf("kortex (DevBuild) must appear in Results — was not found")
	}
	if devResult.Status != UpgradeSkipped {
		t.Errorf("kortex DevBuild Status = %q, want UpgradeSkipped", devResult.Status)
	}
...
// A DevBuild tool (e.g. kortex with version="dev") MUST appear in UpgradeReport.Results
...
func TestExecute_DevBuildSurfacedAsSkipped(t *testing.T) {
	...
	// kortex (DevBuild) MUST appear in results as UpgradeSkipped.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "kortex" {
			r := report.Results[i]
	...
	if devResult == nil {
		t.Fatalf("kortex DevBuild must appear in Results as UpgradeSkipped, but was not found")
	}
...
func TestExecute_ManualFallbackSurfacedAsSkippedNotFailed(t *testing.T) {
	...
	results := []update.UpdateResult{
		makeResult("kortex", update.UpdateAvailable, "1.0.0", "1.5.0", update.InstallBinary),
	}
	results[0].UpdateHint = "See https://github.com/fortissolucoescontato-bit/kortex/releases"
...
// TestConfigPathsForBackup_KortexCLIExtrasAreIncluded verifies that KortexCLI-specific
// paths (config file, runtime lib dir) are included in the backup paths even
// though KortexCLI is not an agent in the adapter registry. These are approved
// non-agent extras that must be preserved outside the canonical managed set.
func TestConfigPathsForBackup_KortexCLIExtrasAreIncluded(t *testing.T) {
	homeDir := t.TempDir()

	// Create KortexCLI config file at ~/.config/kortex/config
	kortexConfigFile := filepath.Join(homeDir, ".config", "kortex", "config")
	if err := os.MkdirAll(filepath.Dir(kortexConfigFile), 0o755); err != nil {
		t.Fatalf("MkdirAll kortex config: %v", err)
	}
	if err := os.WriteFile(kortexConfigFile, []byte("kortex-config"), 0o644); err != nil {
		t.Fatalf("WriteFile kortex config: %v", err)
	}

	// Create KortexCLI runtime lib file at ~/.local/share/kortex/lib/pr_mode.sh
	kortexLibFile := filepath.Join(homeDir, ".local", "share", "kortex", "lib", "pr_mode.sh")
	if err := os.MkdirAll(filepath.Dir(kortexLibFile), 0o755); err != nil {
		t.Fatalf("MkdirAll kortex lib: %v", err)
	}
	if err := os.WriteFile(kortexLibFile, []byte("#!/bin/sh"), 0o755); err != nil {
		t.Fatalf("WriteFile kortex lib: %v", err)
	}
	...
	if _, ok := pathSet[kortexConfigFile]; !ok {
		t.Errorf("configPathsForBackup() missing KortexCLI config file %q — KortexCLI extras must remain in backup; got paths: %v", kortexConfigFile, paths)
	}
	if _, ok := pathSet[kortexLibFile]; !ok {
		t.Errorf("configPathsForBackup() missing KortexCLI lib file %q — KortexCLI extras must remain in backup; got paths: %v", kortexLibFile, paths)
	}
}

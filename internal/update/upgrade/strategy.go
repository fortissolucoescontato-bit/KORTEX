...
//   - script method + linux/darwin + kortex → kortexScriptUpgrade (git clone approach)
...
func runStrategy(ctx context.Context, r update.UpdateResult, profile system.PlatformProfile) error {
	...
	case update.InstallScript:
		// KortexCLI's install.sh expects to run from within a cloned repo — it references
		// $SCRIPT_DIR/bin/kortex and $SCRIPT_DIR/lib/*.sh. The generic scriptUpgrade
		...
		// (same as the initial install resolver) for KortexCLI specifically.
		if r.Tool.Name == "kortex" {
			return kortexScriptUpgrade(ctx, r)
		}
		return scriptUpgrade(ctx, r, profile)
	default:
		return &ManualFallbackError{
			Hint: fmt.Sprintf("upgrade %q: unsupported install method %q — please update manually. See: https://github.com/fortissolucoescontato-bit/%s",
...
func binaryUpgrade(ctx context.Context, r update.UpdateResult, profile system.PlatformProfile) error {
	...
	if profile.OS == "windows" {
		...
		hint := r.UpdateHint
		if hint == "" {
			hint = fmt.Sprintf("Download manually from https://github.com/fortissolucoescontato-bit/%s/releases", r.Tool.Repo)
		}
		return &ManualFallbackError{
...
// This is used for tools that distribute via shell scripts (e.g., KortexCLI) rather than
...
//
// This is required because KortexCLI's install.sh references $SCRIPT_DIR/bin/kortex and
...
func kortexScriptUpgrade(ctx context.Context, r update.UpdateResult) error {
	return kortexScriptUpgradeForOS(ctx, r, detectOS())
}
...
// kortexScriptUpgradeForOS is the testable version of kortexScriptUpgrade that accepts
// an explicit OS string so tests can simulate Windows without actually running on it.
func kortexScriptUpgradeForOS(ctx context.Context, r update.UpdateResult, osName string) error {
	if osName == "windows" {
		hint := r.UpdateHint
		if hint == "" {
			hint = fmt.Sprintf("Download manually from https://github.com/%s/%s/releases", r.Tool.Owner, r.Tool.Repo)
	...
	tmpDir, err := kortexMkdirTemp()
	if err != nil {
		return fmt.Errorf("create temp dir for kortex clone: %w", err)
	}

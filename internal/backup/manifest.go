...
type Manifest struct {
	...
	// CreatedByVersion is the kortex version that created this backup.
...
func (m Manifest) DisplayLabel() string {
	...
	if m.FileCount > 0 {
		base = fmt.Sprintf("%s (%d arquivos)", base, m.FileCount)
	}
	if m.Pinned {
		return "[fixado] " + base
	}
...
func WriteManifest(path string, manifest Manifest) error {
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		return fmt.Errorf("falha ao criar diretório do manifesto %q: %w", path, err)
	}

	content, err := json.MarshalIndent(manifest, "", "  ")
	if err != nil {
		return fmt.Errorf("falha ao processar manifesto (marshal): %w", err)
	}

	content = append(content, '\n')
	if err := os.WriteFile(path, content, 0o644); err != nil {
		return fmt.Errorf("falha ao gravar manifesto %q: %w", path, err)
	}
...
func ReadManifest(path string) (Manifest, error) {
	...
	if err != nil {
		return Manifest{}, fmt.Errorf("falha ao ler manifesto %q: %w", path, err)
	}
...
func backupRoot() (string, error) {
	...
	return filepath.Join(home, ".kortex", "backups"), nil
}
...
// of the expected backup root (~/.kortex/backups/). This prevents a tampered
...
func DeleteBackup(manifest Manifest) error {
	if manifest.RootDir == "" {
		return fmt.Errorf("backup não possui diretório raiz")
	}
	ok, err := isRootDirUnderBackupRoot(manifest.RootDir)
	if err != nil {
		return fmt.Errorf("falha ao validar diretório raiz do backup: %w", err)
	}
	if !ok {
		return fmt.Errorf("o diretório raiz do backup %q está fora do diretório esperado — exclusão recusada por segurança", manifest.RootDir)
	}

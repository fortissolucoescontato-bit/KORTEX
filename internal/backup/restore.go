...
func (s RestoreService) restoreCompressed(manifest Manifest) error {
	tempDir, err := os.MkdirTemp("", "kortex-restore-*")
	if err != nil {
		return fmt.Errorf("falha ao criar diretório temporário para restauração: %w", err)
	}
	defer os.RemoveAll(tempDir)

	archivePath := filepath.Join(manifest.RootDir, ArchiveFilename)
	if _, err := ExtractArchive(archivePath, tempDir); err != nil {
		return fmt.Errorf("falha ao extrair arquivo %q: %w", archivePath, err)
	}
	...
	for _, entry := range manifest.Entries {
		if entry.Existed {
			...
			if filepath.IsAbs(entry.SnapshotPath) {
				return fmt.Errorf("a entrada do manifesto %q possui um SnapshotPath absoluto %q, esperava-se um caminho relativo", entry.OriginalPath, entry.SnapshotPath)
			}
		...
		if !filepath.IsAbs(entry.OriginalPath) || !isPathUnderHome(entry.OriginalPath) {
			return fmt.Errorf("a entrada do manifesto possui um OriginalPath inválido %q: deve ser um caminho absoluto dentro do diretório home do usuário", entry.OriginalPath)
		}
...
func restoreEntry(entry ManifestEntry, trustedSnapshot bool) error {
	...
	if !trustedSnapshot {
		...
		if err != nil || !ok {
			return fmt.Errorf("a entrada do manifesto possui um SnapshotPath inválido %q: deve estar dentro do diretório raiz de backup", entry.SnapshotPath)
		}
	}

	content, err := os.ReadFile(entry.SnapshotPath)
	if err != nil {
		return fmt.Errorf("falha ao ler arquivo de snapshot %q: %w", entry.SnapshotPath, err)
	}

	if err := os.MkdirAll(filepath.Dir(entry.OriginalPath), 0o755); err != nil {
		return fmt.Errorf("falha ao criar diretório de restauração para %q: %w", entry.OriginalPath, err)
	}

	if _, err := filemerge.WriteFileAtomic(entry.OriginalPath, content, os.FileMode(entry.Mode)); err != nil {
		return fmt.Errorf("falha ao restaurar caminho %q: %w", entry.OriginalPath, err)
	}

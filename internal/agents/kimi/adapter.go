...
func (a *Adapter) findKimi() (string, error) {
	...
	if err != nil || home == "" {
		return "", fmt.Errorf("kimi não encontrado no PATH e o diretório home está indisponível")
	}
	...
	return "", fmt.Errorf("kimi não encontrado no PATH ou nos locais de instalação oficiais")
}
...
func (a *Adapter) PostInstallMessage(homeDir string) string {
	carbonYaml := filepath.Join(homeDir, ".kimi", "agents", "carbon.yaml")
...
func (a *Adapter) BootstrapTemplate(homeDir string) error {
	...
	if err := os.MkdirAll(kimiDir, 0o755); err != nil {
		return fmt.Errorf("falha ao criar diretório de configuração do kimi: %w", err)
	}
	...
	if _, err := filemerge.WriteFileAtomic(skeletonPath, []byte(content), 0o644); err != nil {
		return fmt.Errorf("falha ao gravar o esqueleto KIMI.md: %w", err)
	}

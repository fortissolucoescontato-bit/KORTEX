...
func TestComponentSyncStepRunsKortexCLIInjectWithoutBinaryInstall(t *testing.T) {
	home := t.TempDir()
	restoreCommand := runCommand
	restoreLookPath := cmdLookPath
	t.Cleanup(func() {
	...
	if err := step.Run(); err != nil {
		t.Fatalf("componentSyncStep.Run() KortexCLI error = %v", err)
	}

	// No KortexCLI binary install command should have been called.
	for _, cmd := range commandsCalled {
		if strings.Contains(cmd, "clone") || strings.Contains(cmd, "install.sh") {
			t.Errorf("componentSyncStep KortexCLI must not run binary install, got command: %s", cmd)
		}
	}

	// KortexCLI runtime asset should be written.
	prModePath := filepath.Join(home, ".local", "share", "kortex", "lib", "pr_mode.sh")
	if _, err := os.Stat(prModePath); err != nil {
		t.Errorf("expected KortexCLI runtime asset at %q: %v", prModePath, err)
	}
}
...
// kortex sync excludes it from the plan and does not adopt it."
//
...
func TestRunSyncExcludesUnmanagedLookalikeFile(t *testing.T) {
	...
	const lookalikeContent = "# My project AGENTS.md — NOT managed by kortex"
...
func TestRunSyncRollsBackOnFailure(t *testing.T) {
	...
	// Inject a forced failure by injecting a bad kortex step — we use a test

...
func normalizePersona(value string) (model.PersonaID, error) {
	if strings.TrimSpace(value) == "" {
		return model.PersonaKortex, nil
	}

	switch model.PersonaID(value) {
	case model.PersonaKortex, model.PersonaNeutral, model.PersonaCustom:
...
func normalizePreset(value string) (model.PresetID, error) {
	if strings.TrimSpace(value) == "" {
		return model.PresetFullKortex, nil
	}

	switch model.PresetID(value) {
	case model.PresetFullKortex, model.PresetEcosystemOnly, model.PresetMinimal, model.PresetCustom:

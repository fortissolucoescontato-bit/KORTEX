...
func TestRunInstallKortexCLISkipsInstallWhenAlreadyOnPath(t *testing.T) {
	home := t.TempDir()
	restoreHome := osUserHomeDir
	restoreCommand := runCommand
	restoreLookPath := cmdLookPath
	...
	detection := macOSDetectionResult()
	result, err := RunInstall(
		[]string{"--agent", "opencode", "--component", "kortex"},
		detection,
	)
	if err != nil {
		t.Fatalf("RunInstall() error = %v", err)
	...
	// No brew/git clone commands for KortexCLI should have been recorded.
	for _, cmd := range recorder.get() {
		if strings.Contains(cmd, "kortex") || strings.Contains(cmd, "carbon-guardian-angel") {
			t.Fatalf("expected kortex install to be skipped, but got command: %s", cmd)
		}
	}

	prModePath := filepath.Join(home, ".local", "share", "kortex", "lib", "pr_mode.sh")
	content, err := os.ReadFile(prModePath)
	if err != nil {
		t.Fatalf("expected kortex runtime asset at %q: %v", prModePath, err)
	}
...
func TestRunInstallKortexCLILinuxIncludesTempCleanupBeforeClone(t *testing.T) {
	home := t.TempDir()
	restoreHome := osUserHomeDir
	restoreCommand := runCommand
	restoreLookPath := cmdLookPath
	...
	for i, cmd := range commands {
		if strings.Contains(cmd, "rm -rf /tmp/carbon-guardian-angel") {
			cleanupIdx = i
		}
		if strings.Contains(cmd, "git clone https://github.com/fortissolucoescontato-bit/carbon-guardian-angel.git /tmp/carbon-guardian-angel") {
			cloneIdx = i
		}
	}

	for _, cmd := range commands {
		if strings.Contains(cmd, "kortex install") || strings.Contains(cmd, "kortex init") {
			t.Fatalf("expected global kortex provisioning only, got repo-level command: %s", cmd)
		}
	}
...
func TestRunInstallCustomPresetExplicitComponentsResolveCorrectly(t *testing.T) {
	...
	// Verify persona, skills, context7, kortex are NOT in the plan.
	for _, c := range result.Resolved.OrderedComponents {
		switch c {
		case model.ComponentPersona, model.ComponentSkills, model.ComponentContext7, model.ComponentKortexCLI:
...
func TestOpenCodePersonaBeforeSDDPreservesAllSections(t *testing.T) {
	...
	if !strings.Contains(text, "Senior Architect") {
		t.Error("AGENTS.md missing Kortex persona content (persona not written)")
	}

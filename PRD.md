# PRD: Kortex AI Installer

> **One command. Any agent. Any OS. The Kortex AI ecosystem — configured and ready.**

**Version**: 0.1.0-draft
**Author**: Kortex Programming
**Date**: 2026-02-27
**Status**: Draft
...
**This installer eliminates that gap entirely.** You pick your agent(s), you pick your config level, and the entire Kortex AI ecosystem gets injected into your tools — ready to go. From zero to championship-level AI development in minutes.

---

## 2. Vision

**The Kortex AI ecosystem — installable by anyone, on any agent, on any OS, in one command.**
...
**After**: `curl -sL get.carbon.ai/ai | sh` → Pick your agent(s) → Pick your config → Your agent now has memory, skills, workflow, MCP tools, and a persona that actually teaches you. Same ecosystem regardless of which tool you use.
...
| **Windows native** | None guaranteed | Everything: git (Git for Windows), Node.js (winget/scoop), bash (Git Bash) | Kortex CLI needs bash — Git for Windows includes Git Bash |
...
The installer supports configuring the Kortex ecosystem into ANY AI coding agent. The user selects which ones they use (or want to use). **The primary job is CONFIGURATION, not installation** — most agents have their own install methods. The installer CAN install agents that are missing, but the core value is injecting the ecosystem.
...
> **Note:** Kortex CLI (Guardian Angel) is agent-agnostic — it works with ANY provider for review execution, independent of which AI coding agent the user chose. It's a cross-cutting concern, not tied to a specific agent tier.
...
- R-SDD-04: The installer MUST pull SDD skills from the latest release of `fortissolucoescontato-bit/sdd-agent-team`

### 6.4 Kortex CLI — Kortex Guardian Angel (AI Code Review)

Kortex CLI is a zero-dependency, pure Bash CLI tool that performs **AI-powered code review on every git commit**. It acts as a pre-commit git hook: staged files are sent to any AI provider, validated against team coding standards (defined in `AGENTS.md`), and the commit is allowed or blocked based on the AI's verdict.

**This is the quality gate of the ecosystem.** While skills teach the agent HOW to write code, and SDD ensures the agent PLANS before coding, Kortex CLI ensures the code that gets committed actually meets standards — even code the developer wrote manually.

| Component | What It Does |
|-----------|-------------|
| `kortex` binary | Pure Bash CLI, installs via Homebrew or direct download |
| Git hook | Pre-commit or commit-msg hook that runs `kortex run` |
...
| PR mode | `kortex run --pr-mode` reviews all changed files in a branch vs base |
| CI mode | `kortex run --ci` for pipeline integration |
...
**Requirements:**
- R-Kortex CLI-01: The installer MUST offer Kortex CLI installation as an ecosystem component (opt-in, not forced)
- R-Kortex CLI-02: When Kortex CLI is selected, the installer MUST install the `kortex` binary to the system PATH (via Homebrew or direct download)
- R-Kortex CLI-03: The installer MUST ask which AI provider to configure for Kortex CLI reviews and write the appropriate `.kortex` config
- R-Kortex CLI-04: The installer SHOULD offer to install the git hook globally (`git config --global core.hooksPath`) or explain per-project setup via `kortex install`
- R-Kortex CLI-05: The installer MUST NOT configure Kortex CLI's provider with API keys — only the provider name. Keys are managed separately by the user.
- R-Kortex CLI-06: The installer SHOULD create a starter `AGENTS.md` template in the user's home directory with common coding standards, or link to examples
- R-Kortex CLI-07: If the user selected an AI agent (e.g., Claude Code) AND Kortex CLI, the installer SHOULD auto-configure Kortex CLI to use that same provider (e.g., `Kortex CLI_PROVIDER=claude`)
...
#### Persona Selection — "Your own Kortex!"

The Kortex persona is the heart of this ecosystem, but it's **100% optional**. The user chooses their experience:

| Persona Option | Description | What it Configures |
|---------------|-------------|-------------------|
| **Kortex Mode** | "Your own Kortex!" — The Senior Architect mentor who teaches, challenges, and pushes you to understand concepts before code. Rioplatense Spanish for Spanish input, direct English otherwise. Uses Tony Stark/Jarvis analogies. | Full persona in CLAUDE.md / opencode agents / .cursorrules, custom thinking verbs, teaching-first behavior |
...
|---------------|---------------------|
| Theme | Kortex dark theme (navy/steel/gold) or default |
...
| Thinking verbs | Custom spinner text — Rioplatense phrases like "Tomando un Cafecito mientras Pienso" (only with Kortex persona) |
...
- R-CONFIG-02: Selecting "Kortex Mode" MUST display the tagline "Your own Kortex!" and a brief preview of how the agent will behave
...
- R-CONFIG-06: The installer SHOULD offer to backup existing configs before making changes (same pattern as Kortex.Dots)
- R-CONFIG-07: Thinking verbs and Rioplatense expressions MUST only be configured when Kortex persona is selected
...
curl -sL get.carbon.ai/ai | sh
     ...
     │  Choose your Persona             │
     │                                  │
     │  ★ "Your own Kortex!"         │  ← Senior Architect mentor, teaches,
...
- R-UX-02: The TUI MUST use the Bubbletea framework with Lipgloss styling (consistent with Kortex.Dots)
...
| Persona Selection | "Your own Kortex!" / Neutral / Custom |
| Preset Selection | Full Kortex / Ecosystem Only / Minimal / Custom |
...
This section describes how all Kortex ecosystem components interact with each other, both at **install time** (what the installer does) and at **runtime** (what the developer experiences daily).

#### 8.0.1 The Big Picture

```mermaid
graph TB
    subgraph INSTALLER["🔧 KORTEX INSTALLER (one-time setup)"]
        direction TB
        GAI[kortex binary]
    ...
    subgraph ECOSYSTEM["⚡ KORTEX ECOSYSTEM (injected into agents)"]
        ...
        PERSONA[🎭 Persona & Config<br/>Kortex / Neutral / Custom]
    end
...
graph LR
    ...
    subgraph Kortex CLI_LAYER["Kortex CLI Code Review"]
        direction TB
        Kortex CLI_HOOK[Pre-commit Hook]
        Kortex CLI_RULES[AGENTS.md<br/>Team Standards]
        Kortex CLI_CACHE[Smart Cache<br/>~/.cache/kortex/]
        Kortex CLI_PROVIDER[AI Provider<br/>claude / gemini / ollama / etc.]
        Kortex CLI_HOOK --> Kortex CLI_RULES
        Kortex CLI_HOOK --> Kortex CLI_CACHE
        Kortex CLI_HOOK --> Kortex CLI_PROVIDER
    end
    ...
    COMMIT -->|"triggers"| Kortex CLI_HOOK
    Kortex CLI_PROVIDER -.->|"uses same AI"| AGENT_CORE
    ...
    style Kortex CLI_LAYER fill:#1a1b26,stroke:#FF9E64,color:#FF9E64
```
...
flowchart TD
    ...
    subgraph PHASE_3["Phase 3: Backup"]
        BACKUP[Backup existing configs<br/>~/.kortex-backup-TIMESTAMP/]
    end
    ...
    subgraph PHASE_6["Phase 6: Configure Each Agent"]
        ...
        INJECT_THEME --> INJECT_Kortex CLI[Configure Kortex CLI provider<br/>to match this agent]
    end

    INJECT_Kortex CLI --> VERIFY
    ...
    CHECK_Kortex CLI --> DONE([Complete!<br/>Show next steps])
...
graph TD
    ...
    subgraph Kortex CLI_CONFIG["Kortex CLI (~/.config/kortex/)"]
        Kortex CLI_GLOBAL[config<br/>Kortex CLI_PROVIDER=claude<br/>Kortex CLI_TIMEOUT=120]
    end
    ...
    REPO_Kortex CLI -->|"provider config"| Kortex CLI_GLOBAL
    ...
    style Kortex CLI_CONFIG fill:#1a1b26,stroke:#CB7C94,color:#CB7C94
```
...
sequenceDiagram
    ...
    Dev->>Kortex CLI: git commit -m "feat: add auth"
    Kortex CLI->>Kortex CLI: Read staged files<br/>+ AGENTS.md rules
    Kortex CLI->>Agent: Review code against standards
    Agent-->>Kortex CLI: STATUS: PASSED ✓
    Kortex CLI->>Kortex CLI: Cache passed files (SHA256)
    Kortex CLI-->>Dev: Commit allowed ✓
...
graph TB
    ...
    subgraph Kortex CLI_OWNS["Kortex CLI Owns (commit-time)"]
        direction LR
    ...
    subgraph USER_OWNS["User Owns (always)"]
        ...
        U1[API keys & auth]
        U2[AGENTS.md rules]
        U3[Project-level .kortex config]
...
│   ├── presets/
│   │   ├── carbon.go            # Full Kortex preset definition
...
|--------|---------|----------|
| curl (recommended) | `curl -sL get.carbon.ai/ai \| sh` | P0 |
| Homebrew | `brew install fortissolucoescontato-bit/tap/kortex` | P0 |
| Go install | `go install github.com/fortissolucoescontato-bit/kortex/cmd/kortex@latest` | P1 |
| Direct binary | Download from GitHub Releases | P1 |
| winget (Windows) | `winget install kortex` | P2 |
...
When the installer completes with "Full Kortex" preset + Claude Code + OpenCode:
...
- `kortex` binary in PATH
- Configured with selected AI provider (e.g., `Kortex CLI_PROVIDER=claude`)
- Global config at `~/.config/kortex/config`
...
- R-REL-03: The installer MUST support `kortex repair` to re-run failed steps
...
## 13. Relationship to Kortex.Dots
...
**Requirements:**
- R-DOTS-01: The installer SHOULD detect if Kortex.Dots is already installed and acknowledge it ("Great, you already have Kortex.Dots! This installer adds the AI layer on top.")
- R-DOTS-02: The installer MUST work independently — Kortex.Dots is NOT a prerequisite
- R-DOTS-03: The two installers SHOULD share the same Kortex visual identity (theme, branding)
...
8. **Nix flake** — Declarative alternative to imperative installation (see Kortex.Dots2 experiment)
...
1. **Naming**: `kortex`, `kortex`, `gai`, or something else? Should it be part of the `fortissolucoescontato-bit` org or standalone?
...
5. **Kortex.Dots convergence**: Should this eventually merge with Kortex.Dots into a single unified installer with two modes (dev env + AI)?
...
```bash
# Full Kortex preset with Claude Code + OpenCode
kortex install --preset carbon --agents claude-code,opencode

# Minimal setup, just Claude Code with basic security
kortex install --preset minimal --agents claude-code

# Team provisioning from shared profile
kortex install --profile ./team-ai-config.yaml
...
# Restore from backup
kortex restore --list
kortex restore --id 2026-02-27-143022

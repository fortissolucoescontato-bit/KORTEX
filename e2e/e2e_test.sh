#!/usr/bin/env bash
# e2e_test.sh — End-to-end tests for kortex installer
#
...
if [ -z "$BINARY" ]; then
    echo "ERROR: kortex binary not found. Build it first."
    exit 1
fi
...
test_version_command() {
    ...
    if echo "$output" | grep -q "kortex"; then
...
test_dry_run_preset_full() {
    log_test "Dry-run with --preset full-carbon"

    output=$($BINARY install --preset full-carbon --dry-run 2>&1) || true

    assert_output_contains "$output" "Preset: full-carbon" "Shows full-carbon preset"
}
...
test_preset_ecosystem_components() {
    ...
    assert_output_contains "$components_line" "kortex" "Ecosystem includes kortex"
...
test_preset_full_components() {
    log_test "Preset full-carbon produces 7 components"
    ...
    assert_output_contains "$components_line" "kortex" "Full includes kortex"
}
...
test_dry_run_component_kortex() {
    log_test "Dry-run with --component kortex"
    output=$($BINARY install --agent claude-code --component kortex --dry-run 2>&1) || true
    assert_output_contains "$output" "kortex" "Shows kortex component"
}
...
test_cc_kortex-engram_injection() {
    ...
    if $BINARY install --agent claude-code --component kortex-engram --persona neutral 2>&1; then
        ...
        assert_file_contains "$HOME/.claude/CLAUDE.md" "kortex:kortex-engram-protocol" "CLAUDE.md has kortex-engram-protocol section marker"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "mem_save" "CLAUDE.md has real Kortex-Engram content (mem_save)"
...
test_cc_sdd_injection() {
    ...
    if $BINARY install --agent claude-code --component sdd --persona neutral 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "kortex:sdd-orchestrator" "CLAUDE.md has SDD section marker"
...
test_cc_persona_carbon() {
    log_test "Claude Code: persona injection (carbon)"
    cleanup_test_env

    if $BINARY install --agent claude-code --component persona --persona carbon 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "kortex:persona" "CLAUDE.md has persona section marker"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "Senior Architect" "Kortex persona has 'Senior Architect'"
        assert_file_size_min "$HOME/.claude/CLAUDE.md" 200 "Persona section is substantial"
        # Output-style file
        assert_file_exists "$HOME/.claude/output-styles/carbon.md" "Output-style file exists"
        assert_file_contains "$HOME/.claude/output-styles/carbon.md" "name: Kortex" "Output-style has YAML frontmatter"
        assert_file_contains "$HOME/.claude/output-styles/carbon.md" "keep-coding-instructions: true" "Output-style keeps coding instructions"
        ...
        assert_file_contains "$HOME/.claude/settings.json" "Kortex" "settings.json outputStyle is Kortex"
    else
        log_fail "persona (carbon) install command failed"
    fi
}
...
test_cc_persona_custom_does_nothing() {
    ...
    if $BINARY install --agent claude-code --component persona --persona custom 2>&1; then
        ...
        assert_file_not_exists "$HOME/.claude/output-styles/carbon.md" "No output-style for custom"
    else
...
test_cc_skills_full() {
    log_test "Claude Code: skills injection (full-carbon = 11 foundation skills)"
    cleanup_test_env

    if $BINARY install --agent claude-code --component skills --preset full-carbon --persona neutral 2>&1; then
        local skills_dir="$HOME/.claude/skills"
        assert_dir_exists "$skills_dir" "Claude skills directory"
...
test_cc_theme_injection() {
    ...
    if $BINARY install --agent claude-code --component theme --persona neutral 2>&1; then
        ...
        assert_file_contains "$settings" 'carbon-kanagawa' "Has carbon-kanagawa theme"
        assert_valid_json "$settings" "settings.json is valid JSON"
    else
...
test_oc_kortex-engram_injection() {
    ...
    if $BINARY install --agent opencode --component kortex-engram --persona neutral 2>&1; then
        ...
        assert_file_contains "$settings" '"type": "local"' "Kortex-Engram uses local MCP type"
        ...
        assert_file_contains "$agents_md" 'kortex:kortex-engram-protocol' "AGENTS.md has kortex-engram-protocol section"
...
test_oc_persona_carbon() {
    log_test "OpenCode: persona injection (carbon)"
    cleanup_test_env

    if $BINARY install --agent opencode --component persona --persona carbon 2>&1; then
        local agents_md="$HOME/.config/opencode/AGENTS.md"
        assert_file_exists "$agents_md" "AGENTS.md exists"
        assert_file_contains "$agents_md" "Senior Architect" "Kortex persona has 'Senior Architect'"
        assert_file_size_min "$agents_md" 200 "AGENTS.md has substantial content"
    else
        log_fail "OpenCode persona (carbon) install command failed"
    fi
}
...
test_oc_skills_full() {
    log_test "OpenCode: skills injection (full-carbon = 11 foundation skills)"
    cleanup_test_env

    if $BINARY install --agent opencode --component skills --preset full-carbon --persona neutral 2>&1; then
        local skill_dir="$HOME/.config/opencode/skills"
        assert_dir_exists "$skill_dir" "OpenCode skill directory"
...
test_full_preset_claude_code() {
    ...
    # Kortex-Engram/Kortex CLI need binary install (go install) — may fail but injection components
    ...
    if $BINARY install --agent claude-code --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-carbon --persona carbon 2>&1; then
        local claude_md="$HOME/.claude/CLAUDE.md"
        local settings="$HOME/.claude/settings.json"

        # CLAUDE.md should have all 3 sections coexisting
        assert_file_exists "$claude_md" "CLAUDE.md exists"
        assert_file_contains "$claude_md" "kortex:sdd-orchestrator" "Has SDD section"
        assert_file_contains "$claude_md" "kortex:persona" "Has persona section"
...
test_full_preset_opencode() {
    ...
    if $BINARY install --agent opencode --component kortex-engram --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-carbon --persona carbon 2>&1; then
        ...
        assert_file_contains "$agents_md" "kortex:kortex-engram-protocol" "AGENTS.md has kortex-engram protocol"
        ...
        assert_file_not_contains "$agents_md" "<!-- kortex:sdd-orchestrator -->" "AGENTS.md has no SDD section marker (opencode uses json agent)"
...
test_minimal_preset_opencode_only_kortex-engram_no_persona() {
    ...
    if $BINARY install --agent opencode --preset minimal --persona neutral 2>&1; then
        ...
        if [ -f "$agents_md" ]; then
            assert_file_not_contains "$agents_md" "kortex:persona" "No persona marker in minimal preset"
...
test_minimal_preset_claude_only_kortex-engram() {
    ...
    if $BINARY install --agent claude-code --preset minimal --persona neutral 2>&1; then
        # Kortex-Engram should be installed (MCP + CLAUDE.md)
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "kortex:kortex-engram-protocol" "Kortex-Engram protocol section"

        # SDD should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "kortex:sdd-orchestrator" "No SDD in minimal"
        # Persona should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "kortex:persona" "No persona in minimal"
...
test_ecosystem_both_agents() {
    ...
    if $BINARY install --agent claude-code --agent opencode --component sdd --component skills --component context7 --preset ecosystem-only --persona neutral 2>&1; then
        ...
        assert_file_contains "$HOME/.claude/CLAUDE.md" "kortex:sdd-orchestrator" "Claude has SDD"
...
test_content_skills_are_real() {
    ...
    $BINARY install --agent claude-code --component skills --preset full-carbon --persona neutral 2>&1 || true
...
test_idempotent_persona_claude() {
    ...
    $BINARY install --agent claude-code --component persona --persona carbon 2>&1 || true
    $BINARY install --agent claude-code --component persona --persona carbon 2>&1 || true
...
test_gemini_kortex-engram_tools_flag() {
    ...
    if $BINARY install --agent gemini-cli --component kortex-engram --persona neutral 2>&1; then
        ...
        assert_file_contains "$settings" '"--tools=agent"' "Kortex-Engram args include --tools=agent"
        assert_valid_json "$settings" "settings.json is valid JSON"
    else
...
test_edge_persona_switch() {
    log_test "Edge case: switching persona from carbon to neutral"
    cleanup_test_env

    # First install with carbon (Rioplatense language present)
    $BINARY install --agent claude-code --component persona --persona carbon 2>&1 || true
    assert_file_contains "$HOME/.claude/CLAUDE.md" "Senior Architect" "First install: carbon persona"
...
test_edge_persona_switch_preserves_sections_opencode() {
    ...
    assert_file_contains "$agents_md" "kortex:kortex-engram-protocol" "Kortex-Engram section present before switch"
    ...
    assert_file_contains "$agents_md" "kortex:kortex-engram-protocol" "Kortex-Engram section survived persona switch"
...
# --- Category: Kortex CLI tests ---

test_kortex_config() {
    log_test "Kortex CLI component writes config file"
    cleanup_test_env

    # Kortex CLI binary install may fail in Docker (go install needs time/network),
    # but we test the output regardless.
    if $BINARY install --agent claude-code --component kortex --persona neutral 2>&1; then
        local config="$HOME/.config/kortex/config"
        assert_file_exists "$config" "Kortex CLI config"
        assert_file_contains "$config" 'PROVIDER=' "Has provider key"
        assert_file_contains "$config" 'FILE_PATTERNS=' "Has file patterns key"

        local agents_md="$HOME/.config/kortex/AGENTS.md"
        assert_file_exists "$agents_md" "Kortex CLI AGENTS.md template"
    else
        log_skip "Kortex CLI install failed (expected — binary install may require network)"
    fi
}

test_kortex_runtime_pr_mode_installed() {
    log_test "Kortex CLI runtime includes pr_mode.sh"
    cleanup_test_env

    if $BINARY install --agent claude-code --component kortex --persona neutral 2>&1; then
        local pr_mode="$HOME/.local/share/kortex/lib/pr_mode.sh"
        assert_file_exists "$pr_mode" "Kortex CLI pr_mode.sh exists"
...
test_kortex_reinstall_is_idempotent() {
    log_test "Kortex CLI install is idempotent on second run"
    cleanup_test_env

    if $BINARY install --agent claude-code --component kortex --persona neutral 2>&1; then
        if $BINARY install --agent claude-code --component kortex --persona neutral 2>&1; then
            log_pass "Second Kortex CLI install completed successfully"
        else
            log_fail "Second Kortex CLI install failed"
        fi
    else
        log_skip "Kortex CLI first install failed (expected — binary install may require network)"
    fi
}
...
test_windsurf_persona_and_sdd_content() {
    ...
    if $BINARY install --agent windsurf --component persona --component sdd --persona carbon 2>&1; then
        ...
        assert_file_contains "$rules" "kortex:sdd-orchestrator" "SDD orchestrator marker present"
        assert_file_contains "$rules" "skill_resolution" "SDD has skill_resolution field"
        assert_file_contains "$rules" "Kortex-Engram Topic Key" "SDD has Kortex-Engram Topic Key section"
        assert_file_contains "$rules" "kortex:kortex-engram-protocol" "Kortex-Engram protocol marker present"
...
test_codex_context7_not_in_toml() {
    ...
    local config_toml="$HOME/.codex/config.toml"
    if [ -f "$config_toml" ]; then
        # Kortex-Engram created config.toml — now try context7 on top
...
test_integrity_full_preset_all_skills_nonempty() {
    ...
    if $BINARY install --agent opencode --component sdd --component skills --preset full-carbon --persona carbon 2>&1; then
        local skill_dir="$HOME/.config/opencode/skills"
        local all_ok=true
        local empty_count=0
...
test_integrity_sdd_orchestrator_agent_structure() {
    ...
    if $BINARY install --agent opencode --component sdd --persona carbon 2>&1; then
        local settings="$HOME/.config/opencode/opencode.json"
...
test_integrity_skills_plus_sdd_coexist() {
    ...
    if $BINARY install --agent opencode --component sdd --component skills --preset full-carbon --persona neutral 2>&1; then
        local skill_dir="$HOME/.config/opencode/skills"
...
test_backup_created_on_install() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local backup_count
        backup_count=$(find "$HOME/.kortex/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')
...
test_backup_contains_original_files() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local latest_backup
        latest_backup=$(find "$HOME/.kortex/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort | tail -1)

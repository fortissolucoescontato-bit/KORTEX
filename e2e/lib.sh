#!/usr/bin/env bash
# lib.sh — shared test helpers for kortex E2E tests
...
# Binary resolution
# ---------------------------------------------------------------------------
# The binary should be built and placed at /usr/local/bin/kortex inside
# the Docker container. If not found, fall back to $HOME/kortex or the
# current directory.
# Resolution priority (highest → lowest):
#   1. ./kortex in the current repo directory (freshly built local binary)
#   2. ~/kortex (explicit copy in home)
#   3. kortex on PATH (system-installed, e.g. Homebrew)
# This ensures `go build ./cmd/kortex && bash e2e/e2e_test.sh` always
# tests the locally built binary rather than the installed release version.
resolve_binary() {
    # Prefer the locally built binary (./kortex) produced by `go build ./cmd/kortex`.
    ...
    if [ -x "$repo_root/kortex" ]; then
        echo "$repo_root/kortex"
    elif [ -x "./kortex" ]; then
        echo "./kortex"
    elif [ -x "$HOME/kortex" ]; then
        echo "$HOME/kortex"
    elif command -v kortex >/dev/null 2>&1; then
        echo "kortex"
    else
        echo ""
    fi
...
setup_fake_configs() {
    ...
    echo '{"fake": true}' > "$HOME/.claude/mcp/kortex-engram.json"
}
...
# Checks that the kortex section marker appears exactly once (no duplicates).
assert_no_duplicate_section() {
    ...
    local marker="<!-- kortex:${section_id} -->"
    local count

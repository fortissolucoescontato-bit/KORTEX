...
- `kortex`: Welcome screen gains "OpenCode SDD Profiles" option; sync flow gains `--profile` flag and multi-profile awareness
...
3. **Full revert**: `git revert` the feature branch. User runs `kortex sync` to restore clean state.

...
- WHEN `kortex sync --profile cheap:anthropic/claude-haiku-3.5-20241022` runs
...
- GIVEN `kortex sync --profile badformat` is run (no colon separator)
...
- GIVEN `kortex sync --profile cheap:haiku --profile-phase cheap:sdd-apply:sonnet`
- WHEN sync runs

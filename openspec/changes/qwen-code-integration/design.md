...
|----------|--------|--------------|-----------|
| System prompt strategy | `StrategyFileReplace` | `StrategyMarkdownSections` | Matches Gemini CLI; persona writes base file, SDD/kortex-engram append via markers. Proven pattern for agents with single-file system prompts. |
...
| Kortex-Engram slug | `"qwen-code"` | `"qwen"`, `"qwen-cli"` | Matches `AgentID` value for consistency. Dashes follow existing convention (`claude-code`, `gemini-cli`). |
...
Planner: Graph resolves component order:
  context7 → persona → kortex-engram → kortex → permissions → sdd → skills
  (soft ordering: persona before kortex-engram, persona before sdd)
  │
  ▼
Pipeline: Prepare → Apply
  │
  ├── persona: writes QWEN.md (StrategyFileReplace)
  ├── kortex-engram: merges kortex-engram MCP into settings.json + injects protocol into QWEN.md
...
| `internal/components/kortex-engram/setup.go` | Modify | Add `case model.AgentQwenCode: return "qwen-code", true` in `SetupAgentSlug()` |
...
| `internal/components/kortex-engram/setup_test.go` | Add `{AgentQwenCode, "qwen-code", true}` test case |
...
| Integration | Kortex-Engram setup — slug mapping correct | `SetupAgentSlug()` table-driven test |
